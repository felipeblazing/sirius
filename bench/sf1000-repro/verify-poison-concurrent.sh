#!/usr/bin/env bash
# Concurrent poison-on-free gate: SIRIUS_POISON_FREES=1 on a mode=both SF1000 run, so the first
# actor that touches freed device memory faults and the on-exception coredump captures it.
#
# Requires the power/throughput harness (bench/sf1000-repro/run-power.sh, PR #1554) and a
# cucascade build with poison-on-free.
#
# Usage:
...
[ -f "$HERE/run-power.sh" ] || {
  echo "ERROR: bench/sf1000-repro/run-power.sh not found (power/throughput harness, PR #1554)"
  exit 2; }
