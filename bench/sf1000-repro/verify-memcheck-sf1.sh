#!/usr/bin/env bash
# compute-sanitizer sweep of the concurrent (mode=both, staged refresh) workload at SF1, using
# the SF1 pressure config so the SF1000 downgrade/eviction regime fires at toy sizes.
#
# Requires the power/throughput harness (bench/sf1000-repro/run-power.sh, PR #1554).
#
# Usage:
...
# Fresh scratch copy each invocation.
cp -f "$SF1DB" "$SCRATCH"
rm -f "$SCRATCH.wal"
