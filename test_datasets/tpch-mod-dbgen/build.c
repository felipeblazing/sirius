...
gen_phone(DSS_HUGE ind, char *target, long seed)
{
	...
	sprintf(target + 2, "%03d", (int) acode);
	sprintf(target + 6, "%03d", (int) exchg);
	sprintf(target + 10, "%04d", (int) number);
...
mk_cust(DSS_HUGE n_cust, customer_t * c)
{
	...
	(c->mktsegment) = pick_str2(&c_mseg_set, C_MSEG_SD);
...
mk_order(DSS_HUGE index, order_t * o, long upd_num)
{
	...
	static long   *asc_date = NULL;
	char            tmp_str[2];
	// char          **mk_ascdate2 PROTO((void));
	char          *mk_ascdate2 PROTO((void));
	int             delta = 1;
	static int      bInit = 0;
	static char     szFormat[100];
	...
	for (lcnt = 0; lcnt < o->lines; lcnt++)
	{
		...
		(o->l[lcnt].shipinstruct) = pick_str2(&l_instruct_set, L_SHIP_SD);
		(o->l[lcnt].shipmode) = pick_str2(&l_smode_set, L_SMODE_SD);
		...
		// (o->l[lcnt].sdate) = s_date;
		// (o->l[lcnt].cdate) = c_date;
		// (o->l[lcnt].rdate) = r_date;
		(o->l[lcnt].sdate) = asc_date[s_date - STARTDATE];
		(o->l[lcnt].cdate) = asc_date[c_date - STARTDATE];
		(o->l[lcnt].rdate) = asc_date[r_date - STARTDATE];

		if (julian(r_date) <= CURRENTDATE)
		{
			// pick_str(&l_rflag_set, L_RFLG_SD, tmp_str);
			// o->l[lcnt].rflag[0] = *tmp_str;
			(o->l[lcnt].rflag) = pick_str2(&l_rflag_set, L_RFLG_SD);
		}
		else {
			// o->l[lcnt].rflag[0] = 'N';
			(o->l[lcnt].rflag) = 2;
...
mk_supp(DSS_HUGE index, supplier_t * s)
{
	...
	// RANDOM(s->comment, 1, 10000, S_CMNT_SD);
	/*
