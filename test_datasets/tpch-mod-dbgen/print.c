...
pr_line(order_t *o, int mode)
{
    ...
    for (i = 0; i < o->lines; i++)
        ...
        PR_HUGE(fp_l, &o->l[i].rflag);
        PR_HUGE(fp_l, &o->l[i].lstatus);
        PR_HUGE(fp_l, &o->l[i].sdate);
        PR_HUGE(fp_l, &o->l[i].cdate);
        PR_HUGE(fp_l, &o->l[i].rdate);
        // PR_STR(fp_l, o->l[i].shipinstruct, L_INST_LEN);
        // PR_STR(fp_l, o->l[i].shipmode, L_SMODE_LEN);
        PR_HUGE(fp_l, &o->l[i].shipinstruct);
        PR_HUGE(fp_l, &o->l[i].shipmode);
...
//    PR_VSTR(p_fp, part->type,part->tlen);
   PR_HUGE(p_fp, &part->mfgr);
   PR_HUGE(p_fp, &part->brand);
   PR_HUGE(p_fp, &part->type);
   PR_HUGE(p_fp, &part->size);
//    PR_STR(p_fp, part->container, P_CNTR_LEN);
   PR_HUGE(p_fp, &part->container);
