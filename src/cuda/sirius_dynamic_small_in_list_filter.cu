  ...
  : _num_keys(static_cast<std::size_t>(keys.size()))
{
  ...
  if (!domain.has_value() || !supports(keys)) {
    throw std::invalid_argument(
      "[sirius_dynamic_small_in_list_filter] unsupported key column (1..k_max_keys keys of a "
      "membership_key_supported type with no nulls required).");
  }
  ...
  bool const copied = detail::dispatch_key_rep(_domain.rep, [&](auto key_tag) {
    ...
    return detail::with_build_key_iterator<key_type>(_domain, keys, [&](auto first, auto last) {
      thrust::copy(
