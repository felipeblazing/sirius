 ...
 *        sets, cross-carrier mask identity, and DATE / TIMESTAMP keys (native and narrow-carrier
 *        DATE probes, same-unit-only timestamps, zone map as a containment oracle).
 */
...
TEST_CASE("DATE probes propagate their null mask and keep the sentinel conservative",
          "[dynamic_filter][probe][key_domain]")
{
  auto const stream = cudf::get_default_stream();
  auto const mr     = cudf::get_current_device_resource_ref();
  ...
  // Row 1 is null: the mask carries the probe's null mask, as for integer probes.
