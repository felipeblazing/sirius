...
// engine is event-ordered, not host-synced, and read locks are host-scoped, so a downgrade
// must (a) order its conversion behind the producer's writer event and (b) refuse to reclaim a
// batch whose consumers still have reads in flight. Each test parks the producer/reader stream
// with a host function so the race window is deterministic.
          ...
          "[spill_converter_ordering]")
{
  ...
  // Park the producer, then enqueue the final write behind the park. The source must be
  // pinned: a pageable H2D behind a parked stream blocks the host thread.
  delay_state gate;
  ...
  // The batch records its writer event on the producer stream, behind the parked write.
  auto batch = wrap_batch(std::move(col), producer_stream.view());

  // Downgrade on another stream while the producer is still parked. A correctly ordered
  // conversion blocks here, so the gate is released from another thread.
...
// A consumer enqueued reads on its stream and dropped its read lock; the writer event is long
// signaled, so only the recorded reader event stands between the downgrade and the free.
          ...
          "[spill_converter_ordering]")
{
  ...
  // Consumer: lock, park the stream, enqueue the read behind the park, publish it, unlock.
  delay_state gate;
  ...
  // Non-blocking downgrade must refuse while the read is in flight.
  sirius::convertible_data_batch wrapper(batch);
  REQUIRE_FALSE(
    wrapper.convert({env().host_space}, downgrade_stream.view(), *env().mgr, false).has_value());
  REQUIRE_FALSE(batch->try_to_mutable().has_value());

  // Blocking downgrade must wait for the read, then run the full downgrade shape: convert,
  // free the source, and force VA reuse with a poison fill on the downgrade stream.
