...
1. **Writer-event wait at conversion reads** (cucascade `d1957ec`): every GPU-reading
...
2. **Reader-fencing `rebind_stream`** (cucascade `445a9b0`): `gpu_table_representation` tracks
...
|---|------|--------------------|
| 1 | Reservation-adaptor `deallocate` → upstream `cudaFreeAsync` | Stream-ordered on the buffer's bound stream (RMM/cudaMallocAsync semantics); reader side covered by the rebind lineage (cucascade `445a9b0`) |
...
| 16 | Quarantine drain (diagnostics, cucascade `58e795c`) | `cudaEventSynchronize` on the free-stream event before the upstream release |
